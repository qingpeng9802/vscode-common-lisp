...
import { addToDictArr, checkDefName, getValidGroup, isRangeIntExcludedRanges } from './collect_symbol_util';
...
function collectKeyword(regex: RegExp, nameGroup: number[], document: vscode.TextDocument, ExcludedRange: [number, number][]): Record<string, SymbolInfo[]> {
  ...
  const matchRes = matchAllWithIndices(text, regex);
...
function collectLocalDef(document: vscode.TextDocument, ExcludedRange: [number, number][]):
  ...
  const matchRes = matchAllWithIndices(text, /(?<=#'|\s|^)(\()(\s*)(macrolet|labels|flet)\s+(\()\s*(\()\s*([#:A-Za-z0-9\+\-\*\/\@\$\%\^\&\_\=\<\>\~\!\?\[\]\{\}\.]+?)((\s|\)))\s*(\()/igmd);
