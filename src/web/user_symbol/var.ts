...
import { addToDictArr, checkDefName, isRangeIntExcludedRanges } from './collect_symbol_util';
import { processVars } from './lambda_list';
import { matchAllWithIndices } from './regex_indices.js';
...
function collectKeywordVars(document: vscode.TextDocument, ExcludedRange: [number, number][]): Record<string, SymbolInfo[]> {
  ...
  const matchRes1 = matchAllWithIndices(text, /(?<=#'|\s|^)(\()(\s*)(lambda|let\*|let|symbol-macrolet|do\*|do|prog\*|prog|multiple-value-bind|destructuring-bind)\s+?(\()/igmd);
...
function collectKeywordSingleVar(document: vscode.TextDocument, ExcludedRange: [number, number][]): Record<string, SymbolInfo[]> {
  ...
  const matchRes2 = matchAllWithIndices(text, /(?<=#'|\s|^)(\()(\s*)(do-symbols|do-external-symbols|do-all-symbols|dolist|dotimes|pprint-logical-block|with-input-from-string|with-open-file|with-open-stream|with-output-to-string|with-package-iterator|with-hash-table-iterator)\s+?(\()\s*?([#:A-Za-z0-9\+\-\*\/\@\$\%\^\&\_\=\<\>\~\!\?\[\]\{\}\.]+)(\s|\)|\()/igmd);
